...
public class ElytraFlight extends Module
{
    ...
    public void onSendPacket(final PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketPlayer && this.mode.getValue() == Mode.TOOBEE) {
            ...
            if (ElytraFlight.mc.player.isElytraFlying() && !ElytraFlight.mc.player.movementInput.jump && packet.pitch < 1.0f) {
                packet.pitch = 1.0f;
            }
        }
    }
        ...
        switch (event.getStage()) {
            case 0: {
                ...
                if (this.mode.getValue() == Mode.BETTER) {
                    final double[] dir = MathUtil.directionSpeed((this.devMode.getValue() == 1) ? this.speed.getValue() : this.hSpeed.getValue());
