...
public class HUD extends Module
{
    ...
    public HUD() {
        ...
        this.spoofGreeter = (Setting<String>)this.register(new Setting("GreeterName", (T)"3arthqu4ke", v -> this.greeter.getValue() == Greeter.CUSTOM));
        ...
        this.sound = (Setting<Sound>)this.register(new Setting("Sound", (T)Sound.NONE, v -> this.hitMarkers.getValue()));
        ...
        this.grayNess = (Setting<Boolean>)this.register(new Setting("FutureColour", (T)true));
    ...
    public void onSendPacket(final PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketUseEntity && event.getPacket().getAction() == CPacketUseEntity.Action.ATTACK) {
            this.shouldIncrement = true;
            switch (this.sound.getValue()) {
                case COD: {
                    HUD.mc.world.playSound((EntityPlayer)HUD.mc.player, HUD.mc.player.posX, HUD.mc.player.posY, HUD.mc.player.posZ, HUD.COD_EVENT, SoundCategory.PLAYERS, 3.0f, 1.0f);
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case PHOBOS: {
                this.renderer.drawString("Phobos" + (this.modeVer.getValue() ? " v1.5.3" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? ((int)this.colorMap.get(2)) : this.color, true);
                break;
            }
            case EARTH: {
                this.renderer.drawString("3arthh4ck" + (this.modeVer.getValue() ? " v1.5.3" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? ((int)this.colorMap.get(2)) : this.color, true);
                break;
            }
        }
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Phobos.moduleManager.alphabeticallySortedModules.size() : Phobos.moduleManager.sortedModules.size()); ++k) {
                    ...
                    this.renderer.drawString(text, width - 2 - this.renderer.getStringWidth(text) + module.arrayListOffset, (float)(2 + j * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? ((int)this.colorMap.get(MathUtil.clamp(2 + j * 10, 0, height))) : this.color, true);
                    ++j;
                }
            }
            else {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Phobos.moduleManager.alphabeticallySortedModules.size() : Phobos.moduleManager.sortedModules.size()); ++k) {
                    ...
                    final float x = width - 2 - this.renderer.getStringWidth(text) + module.arrayListOffset;
                    final int n = height;
                    j += 10;
        ...
        if (this.renderingUp.getValue()) {
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : Phobos.potionManager.getOwnPotions()) {
                    final String text3 = Phobos.potionManager.getColoredPotionString(effect);
