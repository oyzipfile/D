    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        Tracer.mc.world.loadedEntityList.stream().filter(EntityUtil::isLiving).filter(entity -> {
            if (entity instanceof EntityPlayer) {
                booleanValue = (this.players.getValue() && Tracer.mc.player != entity);
            }
            else {
                booleanValue = (EntityUtil.isPassive(entity) ? this.animals.getValue() : this.mobs.getValue());
            }
            return booleanValue;
        }).filter(entity -> Tracer.mc.player.getDistanceSq(entity) < MathUtil.square(this.distance.getValue())).filter(entity -> this.invisibles.getValue() || !entity.isInvisible()).forEach(entity -> {
