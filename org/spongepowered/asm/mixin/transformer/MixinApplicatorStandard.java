...
class MixinApplicatorStandard
{
    ...
    void apply(final SortedSet<MixinInfo> mixins) {
        ...
        try {
            final Iterator<MixinTargetContext> iterator2 = mixinContexts.iterator();
            while (iterator2.hasNext()) {
                final MixinTargetContext context = current = iterator2.next();
            ...
            for (final ApplicatorPass pass : ApplicatorPass.values()) {
                ...
                final Iterator<MixinTargetContext> iterator3 = mixinContexts.iterator();
                while (iterator3.hasNext()) {
                    final MixinTargetContext context2 = current = iterator3.next();
                    this.applyMixin(current, pass);
                }
                timer.end();
            }
            final Iterator<MixinTargetContext> iterator4 = mixinContexts.iterator();
            while (iterator4.hasNext()) {
                final MixinTargetContext context = current = iterator4.next();
    ...
    protected final void appendInsns(final MixinTargetContext mixin, final MethodNode method) {
        ...
        if (target != null) {
            ...
            if (returnNode != null) {
                ...
                final MethodNode methodNode = target;
                methodNode.maxLocals = Math.max(methodNode.maxLocals, method.maxLocals);
                final MethodNode methodNode2 = target;
                methodNode2.maxStack = Math.max(methodNode2.maxStack, method.maxStack);
            }
            return;
        }
        this.targetClass.methods.add(method);
    ...
    protected void applyInitialisers(final MixinTargetContext mixin) {
        ...
        for (final MethodNode method : this.targetClass.methods) {
            if ("<init>".equals(method.name)) {
                ...
                methodNode.maxStack = Math.max(methodNode.maxStack, ctor.maxStack);
